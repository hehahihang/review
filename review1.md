...
- Java에서 Stack은 Class를 이용하여 구현하여 제공지만, Queue는 별도의 클래스가 없고 Queue 인터페이스를 구현한 클래스들을 사용해야한다. 
  - ex)LinkedList 
- Array와 List로 구현하는 것의 차이는, **처음 크기를 할당하는지 or 사용할 때마다 크기를 조절하는지** 에 따라 다를 것으로 예상했음  

* Stack 구현 방법 2가지 (배열, 연결리스트)  
  1. **배열(Array)**
    > 장점 : 랜덤 엑세스가 빠름, 빠르게 데이터에 접근이 가능
    > 단점 : 메모리 사용 비효율적, 데이터 이동 및 재구성 어려움  
  2. **연결리스트(LinkedList)**
    > 장점 : 동적으로 메모리 사용 가능, 메모리 효율적 사용, 대용량 데이터 처리 적합
    > 단점 : 특정 위치 데이터 검색시 느림(엑세스 느림), 메모리를 추가적으로 사용해야함  
...
* REST를 사용하는 이유?
  ...
  - 다양한 브라우저와 모바일기기(Android, iOS) 에서도 통신할 수 있어야하기 때문<br><br>
* REST의 장단점
  - 장점
  ...
  - 단점
    1. 표준이 존재하지 않음
    2. 사용할 수 있는 Method가 4가지 밖에 없다<br><br>
* REST의 특징  
  1. **Client - Server 구조** 
